...
function TelaInspecao({ pop, tecnico, ultimosCheckIns, listaPops, onSelectPop, onBack, onCheckInRealizado, darkMode, setDarkMode, theme }) {
  ...
  const nomePopLower = pop && pop.nome ? pop.nome.toLowerCase() : '';
  ...
  const docRef = pop && pop.nome ? doc(db, "pops_dados", pop.nome) : null;
  ...
  const linkGoogleMaps = pop && pop.nome ? (mapasPops[pop.nome.toLowerCase()] || null) : null;

  useEffect(() => {
    if (!pop || !pop.nome || !docRef) return;
    const carregarDadosPop = async () => {
      try {
        ...
        if (snap.exists()) {
          ...
          if (data.fotosPop && Array.isArray(data.fotosPop)) setFotosPop(data.fotosPop);
          ...
          if (data.statusAtivos) {
            ...
            setStatusAtivos(prev => ({ ...prev, ...filtrados }));
          }
          if (data.ativosPresentes) {
            ...
            setAtivosPresentes(prev => ({ ...prev, ...filtradosPres }));
          }
          if (data.detalhesIncidentes) setDetalhesIncidentes(data.detalhesIncidentes);
          if (data.incidentesGerais !== undefined) setIncidentesGerais(data.incidentesGerais || '');
          if (data.precisaLimpeza !== undefined) setPrecisaLimpeza(data.precisaLimpeza);
          if (data.anotacoes !== undefined) setAnotacoes(data.anotacoes || '');
          ...
          const loadedBancos = {};
          const qtdB = data.qtdBancos || 1;
          for (let i = 1; i <= qtdB; i++) {
            loadedBancos[i] = {
          ...
          const loadedAr = {};
          const qtdA = data.qtdAr || 1;
          for (let i = 1; i <= qtdA; i++) {
            loadedAr[i] = {
        ...
        console.error("Erro ao carregar dados do POP:", err);
      }
    };
  ...
  const obterDadosCheckInOriginal = () => {
    if (ultimosCheckIns && ultimosCheckIns.length > 0 && pop && pop.nome) {
    ...
    if (janelaPdf) {
      ...
      Array.from({ length: qtdBancos }, (_, i) => i + 1).forEach(banco => {
        ...
        if (bModel) {
          ...
          const voltagensHtml = bModel.tipo !== 'Lítio' && Array.isArray(bModel.voltagens)
          ...
          htmlRelatorio += `
            <div class="bloco">
              <span class="negrito">Banco ${getLetra(banco)} (${bModel.tipo || 'Chumbo'})</span><br>
  ...
  if (!pop || !pop.nome) {
    return <div style={{ color: theme.textMain, backgroundColor: theme.bg, padding: '20px' }}>Nenhum POP selecionado. <button onClick={onBack}>Voltar</button></div>;
  }
    ...
    <div className="container-movel" style={{ backgroundColor: theme.bg, color: theme.textMain, minHeight: '100vh', width: '100%', margin: 0, padding: '15px 10px', fontFamily: 'sans-serif', boxSizing: 'border-box', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
      ...
      <div style={{ width: '100%', maxWidth: '900px', boxSizing: 'border-box' }}>
        ...
        <div className="card-movel" style={{ background: theme.cardBg, color: theme.textMain, padding: '15px', width: '100%', borderRadius: '6px', border: `1px solid ${theme.border}`, boxSizing: 'border-box' }}>
                  ...
                  <label style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer', fontSize: '16px', fontWeight: 'bold' }}>
                    <input 
                      ...
                      onChange={(e) => {
                        ...
                        const novosPres = { ...ativosPresentes, [ativo]: val };
                        setAtivosPresentes(novosPres);
                        salvarNoFirebase({ ativosPresentes: novosPres });
                      }} 
                    />
                    {ativo}
                  </label>
                  {presente && (
                    <div style={{ display: 'flex', gap: '6px' }}>
                      <button 
                        type="button"
                        onClick={() => {
                          const novoStatus = { ...statusAtivos, [ativo]: 'OK' };
                          setStatusAtivos(novoStatus);
                          const novosDet = { ...detalhesIncidentes };
                          delete novosDet[ativo];
                          setDetalhesIncidentes(novosDet);
                          salvarNoFirebase({ statusAtivos: novoStatus, detalhesIncidentes: novosDet });
                        }}
                        style={{ padding: '6px 12px', background: statusAtivos[ativo] === 'OK' ? '#28a745' : theme.cardBg, color: statusAtivos[ativo] === 'OK' ? '#fff' : theme.textMain, border: `1px solid ${theme.border}`, borderRadius: '4px', cursor: 'pointer', fontWeight: 'bold', fontSize: '13px' }}
                      >
                        OK
                      </button>
                      <button 
                        type="button"
                        onClick={() => {
                          const novoStatus = { ...statusAtivos, [ativo]: 'Incidente' };
                          setStatusAtivos(novoStatus);
                          salvarNoFirebase({ statusAtivos: novoStatus });
                        }}
                        style={{ padding: '6px 12px', background: statusAtivos[ativo] === 'Incidente' ? '#dc3545' : theme.cardBg, color: statusAtivos[ativo] === 'Incidente' ? '#fff' : theme.textMain, border: `1px solid ${theme.border}`, borderRadius: '4px', cursor: 'pointer', fontWeight: 'bold', fontSize: '13px' }}
                      >
                        Incidente
                      </button>
                    </div>
                ...
                {presente && statusAtivos[ativo] === 'Incidente' && (
                  <div style={{ marginTop: '10px' }}>
                    <input 
                      ...
                      value={detalhesIncidentes[ativo] || ''} 
                      onChange={(e) => {
                        const novosDet = { ...detalhesIncidentes, [ativo]: e.target.value };
                        setDetalhesIncidentes(novosDet);
                        salvarNoFirebase({ detalhesIncidentes: novosDet });
                      }}
                      style={{ width: '100%', padding: '8px', background: theme.inputBg, border: `1px solid ${theme.border}`, color: theme.inputText, boxSizing: 'border-box', fontSize: '14px', borderRadius: '4px' }}
                    />
                  </div>
                )}
              </div>
          ...
          <div style={{ background: theme.cardInner, padding: '12px', borderRadius: '6px', marginBottom: '15px', border: `1px solid ${theme.border}`, boxSizing: 'border-box' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
              <h3 style={{ fontSize: '18px', margin: 0, color: '#4dabf7' }}>🔋 Bancos de Baterias</h3>
              <div style={{ display: 'flex', gap: '6px' }}>
                <select 
                  value={qtdBancos} 
                  onChange={(e) => {
                    ...
                    setQtdBancos(q);
                    salvarNoFirebase({ qtdBancos: q });
                  }}
                  style={{ padding: '5px', background: theme.inputBg, color: theme.inputText, border: `1px solid ${theme.border}`, borderRadius: '4px', fontSize: '13px' }}
                >
                  {[1, 2, 3, 4, 5, 6].map(n => <option key={n} value={n}>{n} Banco(s)</option>)}
                </select>
              </div>
            </div>

            {Array.from({ length: qtdBancos }, (_, i) => i + 1).map((bancoIdx) => {
              const bancoObj = bancosBateria[bancoIdx] || { tipo: 'Chumbo', dataFabricacao: '', dataUltimaInspecao: '', voltagens: ['', '', '', ''], salvo: false };
              const anosTrocaCalculado = (bancoObj.tipo && bancoObj.tipo.toLowerCase() === 'lítio') ? 8 : 2;
              const { textoExato } = parseDataFabricacaoBateria(bancoObj.dataFabricacao);
              const proximaSub = calcularProximaSubstituicaoBateria(bancoObj.dataFabricacao, pop.nome, bancoObj.tipo);
              const resSub = statusData(proximaSub);
              const vencidoSub = resSub && resSub.status === 'vencido';

              const proxInsp = calcularProximaInspecaoGeral(bancoObj.dataUltimaInspecao);
              const resInsp = statusData(proxInsp);
              const vencidoInsp = resInsp && resInsp.status === 'vencido';

              return (
                <div key={bancoIdx} style={{ background: theme.cardBg, padding: '12px', borderRadius: '6px', marginBottom: '12px', border: `1px solid ${theme.border}` }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                    <h4 style={{ margin: 0, color: '#4dabf7', fontSize: '15px' }}>Banco {getLetra(bancoIdx)}</h4>
                    <button 
                      type="button"
                      onClick={async () => {
                        const dadosBancoSalvar = {
                          [`bat_${bancoIdx}_tipo`]: bancoObj.tipo,
                          [`bat_${bancoIdx}_fab`]: bancoObj.dataFabricacao,
                          [`bat_${bancoIdx}_insp`]: bancoObj.dataUltimaInspecao,
                          [`bat_${bancoIdx}_v1`]: bancoObj.voltagens[0],
                          [`bat_${bancoIdx}_v2`]: bancoObj.voltagens[1],
                          [`bat_${bancoIdx}_v3`]: bancoObj.voltagens[2],
                          [`bat_${bancoIdx}_v4`]: bancoObj.voltagens[3],
                          [`bat_${bancoIdx}_salvo`]: true
                        };
                        const novosBancos = { ...bancosBateria, [bancoIdx]: { ...bancoObj, salvo: true } };
                        setBancosBateria(novosBancos);
                        await salvarNoFirebase(dadosBancoSalvar);
                        alert(`Banco ${getLetra(bancoIdx)} salvo com sucesso!`);
                      }}
                      style={{ background: '#28a745', border: 'none', color: '#fff', padding: '5px 10px', borderRadius: '4px', cursor: 'pointer', fontSize: '13px', fontWeight: 'bold' }}
                    >
                      Salvar Banco {getLetra(bancoIdx)}
                    </button>
                  </div>

                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', marginBottom: '8px' }}>
                    <div>
                      <label style={{ display: 'block', fontSize: '12px', color: theme.textMuted, marginBottom: '2px' }}>Tipo</label>
                      <select 
                        value={bancoObj.tipo || 'Chumbo'} 
                        onChange={(e) => {
                          const tipoVal = e.target.value;
                          setBancosBateria({ ...bancosBateria, [bancoIdx]: { ...bancoObj, tipo: tipoVal } });
                        }}
                        style={{ width: '100%', padding: '7px', background: theme.inputBg, color: theme.inputText, border: `1px solid ${theme.border}`, borderRadius: '4px', fontSize: '13px' }}
                      >
                        <option value="Chumbo">Chumbo (+2 anos)</option>
                        <option value="Lítio">Lítio (+8 anos)</option>
                      </select>
                    </div>

                    <div>
                      <label style={{ display: 'block', fontSize: '12px', color: theme.textMuted, marginBottom: '2px' }}>Data de Fabricação</label>
                      <input 
                        type="text" 
                        placeholder="dd/MM/aaaa ou sem/ano" 
                        value={bancoObj.dataFabricacao || ''} 
                        onChange={(e) => {
                          setBancosBateria({ ...bancosBateria, [bancoIdx]: { ...bancoObj, dataFabricacao: e.target.value } });
                        }}
                        style={{ width: '100%', padding: '7px', background: theme.inputBg, color: theme.inputText, border: `1px solid ${theme.border}`, borderRadius: '4px', fontSize: '13px', boxSizing: 'border-box' }}
                      />
                    </div>
                  </div>

                  {textoExato && (
                    <p style={{ fontSize: '12px', color: '#4dabf7', margin: '0 0 6px 0' }}>Fabricação estimada: {textoExato}</p>
                  )}

                  {proximaSub && (
                    <p className={vencidoSub ? 'alerta-vencido' : ''} style={{ fontSize: '13px', margin: '0 0 8px 0', color: vencidoSub ? undefined : '#28a745', fontWeight: 'bold' }}>
                      Próxima Substituição (+{anosTrocaCalculado} anos): {proximaSub} {vencidoSub ? `(Expirado há ${resSub.dias} dias)` : ''}
                    </p>
                  )}

                  <div style={{ marginBottom: '8px' }}>
                    <label style={{ display: 'block', fontSize: '12px', color: theme.textMuted, marginBottom: '2px' }}>Data da Última Inspeção de Bateria (dd/MM/aaaa)</label>
                    <input 
                      type="text" 
                      placeholder="dd/MM/aaaa" 
                      value={bancoObj.dataUltimaInspecao || ''} 
                      onChange={(e) => {
                        setBancosBateria({ ...bancosBateria, [bancoIdx]: { ...bancoObj, dataUltimaInspecao: e.target.value } });
                      }}
                      style={{ width: '100%', padding: '7px', background: theme.inputBg, color: theme.inputText, border: `1px solid ${theme.border}`, borderRadius: '4px', fontSize: '13px', boxSizing: 'border-box' }}
                    />
                    {proxInsp && (
                  ...
                  {bancoObj.tipo !== 'Lítio' && (
                    <div style={{ marginTop: '8px' }}>
                      <label style={{ display: 'block', fontSize: '12px', color: theme.textMuted, marginBottom: '4px', fontWeight: 'bold' }}>Voltagens Individuais das Baterias</label>
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '6px' }}>
                        {[0, 1, 2, 3].map(vIdx => (
                          <input 
                            ...
                            value={Array.isArray(bancoObj.voltagens) ? (bancoObj.voltagens[vIdx] || '') : ''}
                            onChange={(e) => {
                              const novasV = Array.isArray(bancoObj.voltagens) ? [...bancoObj.voltagens] : ['', '', '', ''];
                              novasV[vIdx] = e.target.value;
                              setBancosBateria({ ...bancosBateria, [bancoIdx]: { ...bancoObj, voltagens: novasV } });
                            }}
                            style={{ width: '100%', padding: '6px', background: theme.inputBg, color: theme.inputText, border: `1px solid ${theme.border}`, borderRadius: '4px', fontSize: '12px', boxSizing: 'border-box', textAlign: 'center' }}
                          />
                        ))}
                      </div>
                    </div>
          ...
          <div style={{ background: theme.cardInner, padding: '12px', borderRadius: '6px', marginBottom: '15px', border: `1px solid ${theme.border}`, boxSizing: 'border-box' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
              <h3 style={{ fontSize: '18px', margin: 0, color: '#4dabf7' }}>❄️ Centrais de Ar Condicionado</h3>
              <select 
                value={qtdAr} 
                onChange={(e) => {
                  const q = parseInt(e.target.value, 10);
                  setQtdAr(q);
                  salvarNoFirebase({ qtdAr: q });
                }}
                style={{ padding: '5px', background: theme.inputBg, color: theme.inputText, border: `1px solid ${theme.border}`, borderRadius: '4px', fontSize: '13px' }}
              >
                {[1, 2, 3, 4].map(n => <option key={n} value={n}>{n} Central(is)</option>)}
              </select>
            </div>

            {Array.from({ length: qtdAr }, (_, i) => i + 1).map((arIdx) => {
              const arObj = centraisAr[arIdx] || { modelo: '', btu: '', dataInstalacao: '', dataUltimaLimpeza: '', salvo: false };
              const proxLimp = calcularProximaLimpezaAr(arObj.dataUltimaLimpeza, intervaloAr);
              const resLimp = statusData(proxLimp);
              const vencidoLimp = resLimp && resLimp.status === 'vencido';

              return (
                <div key={arIdx} style={{ background: theme.cardBg, padding: '12px', borderRadius: '6px', marginBottom: '12px', border: `1px solid ${theme.border}` }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                    <h4 style={{ margin: 0, color: '#4dabf7', fontSize: '15px' }}>Central {getLetra(arIdx)}</h4>
                    <button 
                      type="button"
                      onClick={async () => {
                        const dadosArSalvar = {
                          [`ar_${arIdx}_mod`]: arObj.modelo,
                          [`ar_${arIdx}_btu`]: arObj.btu,
                          [`ar_${arIdx}_inst`]: arObj.dataInstalacao,
                          [`ar_${arIdx}_limp`]: arObj.dataUltimaLimpeza,
                          [`ar_${arIdx}_salvo`]: true
                        };
                        const novoAr = { ...centraisAr, [arIdx]: { ...arObj, salvo: true } };
                        setCentraisAr(novoAr);
                        await salvarNoFirebase(dadosArSalvar);
                        alert(`Central ${getLetra(arIdx)} salva com sucesso!`);
                      }}
                      style={{ background: '#28a745', border: 'none', color: '#fff', padding: '5px 10px', borderRadius: '4px', cursor: 'pointer', fontSize: '13px', fontWeight: 'bold' }}
                    >
                      Salvar Central {getLetra(arIdx)}
                    </button>
                  </div>

                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', marginBottom: '8px' }}>
                    <div>
                      <label style={{ display: 'block', fontSize: '12px', color: theme.textMuted, marginBottom: '2px' }}>Modelo (ex: Elgin)</label>
                      <input 
                        type="text" 
                        placeholder="Ex: Elgin" 
                        value={arObj.modelo || ''} 
                        onChange={(e) => {
                          setCentraisAr({ ...centraisAr, [arIdx]: { ...arObj, modelo: e.target.value } });
                        }}
                        style={{ width: '100%', padding: '7px', background: theme.inputBg, color: theme.inputText, border: `1px solid ${theme.border}`, borderRadius: '4px', fontSize: '13px', boxSizing: 'border-box' }}
                      />
                    </div>
                    <div>
                      <label style={{ display: 'block', fontSize: '12px', color: theme.textMuted, marginBottom: '2px' }}>BTU</label>
                      <input 
                        type="text" 
                        placeholder="Ex: 12000" 
                        value={arObj.btu || ''} 
                        onChange={(e) => {
                          setCentraisAr({ ...centraisAr, [arIdx]: { ...arObj, btu: e.target.value } });
                        }}
                        style={{ width: '100%', padding: '7px', background: theme.inputBg, color: theme.inputText, border: `1px solid ${theme.border}`, borderRadius: '4px', fontSize: '13px', boxSizing: 'border-box' }}
                      />
                    </div>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', marginBottom: '4px' }}>
                    <div>
                      <label style={{ display: 'block', fontSize: '12px', color: theme.textMuted, marginBottom: '2px' }}>Data Instalação</label>
                      <input 
                        type="text" 
                        placeholder="dd/MM/aaaa" 
                        value={arObj.dataInstalacao || ''} 
                        onChange={(e) => {
                          setCentraisAr({ ...centraisAr, [arIdx]: { ...arObj, dataInstalacao: e.target.value } });
                        }}
                        style={{ width: '100%', padding: '7px', background: theme.inputBg, color: theme.inputText, border: `1px solid ${theme.border}`, borderRadius: '4px', fontSize: '13px', boxSizing: 'border-box' }}
                      />
                    </div>
                    <div>
                      <label style={{ display: 'block', fontSize: '12px', color: theme.textMuted, marginBottom: '2px' }}>Última Limpeza</label>
                      <input 
                        type="text" 
                        placeholder="dd/MM/aaaa" 
                        value={arObj.dataUltimaLimpeza || ''} 
                        onChange={(e) => {
                          setCentraisAr({ ...centraisAr, [arIdx]: { ...arObj, dataUltimaLimpeza: e.target.value } });
                        }}
                        style={{ width: '100%', padding: '7px', background: theme.inputBg, color: theme.inputText, border: `1px solid ${theme.border}`, borderRadius: '4px', fontSize: '13px', boxSizing: 'border-box' }}
                      />
                    </div>
          ...
          <div style={{ background: theme.cardInner, padding: '12px', borderRadius: '6px', marginBottom: '15px', border: `1px solid ${theme.border}`, boxSizing: 'border-box' }}>
            <h3 style={{ fontSize: '18px', margin: '0 0 10px 0', color: '#4dabf7' }}>📝 Observações e Incidentes Gerais</h3>
            
            <div style={{ marginBottom: '10px' }}>
              <label style={{ display: 'block', fontSize: '13px', color: theme.textMuted, marginBottom: '3px' }}>Incidentes Gerais / Problemas no POP</label>
              <textarea 
                rows={3} 
                value={incidentesGerais || ''} 
                onChange={(e) => setIncidentesGerais(e.target.value)} 
                placeholder="Descreva problemas gerais..." 
                style={{ width: '100%', padding: '8px', background: theme.inputBg, border: `1px solid ${theme.border}`, color: theme.inputText, boxSizing: 'border-box', fontSize: '14px', borderRadius: '4px' }} 
              />
            </div>
            ...
            <div style={{ marginBottom: '10px', display: 'flex', alignItems: 'center', gap: '8px' }}>
              ...
              />
              <label htmlFor="chkLimpeza" style={{ cursor: 'pointer', fontSize: '14px', fontWeight: 'bold', color: precisaLimpeza ? '#ff4d4d' : theme.textMain }}>
                POP Precisa de Limpeza Geral / Organização
              </label>
            </div>
            ...
            <button 
              type="button" 
              onClick={salvarStatusAtivosFirebase} 
              style={{ width: '100%', padding: '10px', background: '#007bff', border: 'none', color: '#fff', fontWeight: 'bold', borderRadius: '4px', cursor: 'pointer', fontSize: '14px' }}
            >
              Salvar Observações e Status
            </button>
          </div>
          ...
          <button 
            ...
            style={{ width: '100%', padding: '14px', background: '#28a745', border: 'none', color: '#fff', fontSize: '16px', fontWeight: 'bold', borderRadius: '6px', cursor: 'pointer' }}
          >
